...
1. There was one component, [ext_server_sniffer][], which wouldn't build out of the box because of a missing dependency. This dependency, the packet sniffer SDK, is one that's available to Rapid7 employees only. With this dependency missing, the build would finish with errors.
...
So after a few weeks and lots of commits, I took the plunge and finalised the last of the issues with warnings, passed on a [pull request][clean_builds_pr] which the legenday [Juan][] landed, resulting in this ...

![Clean Builds][clean_builds_image]

Clean builds make for a happy OJ. And with that locked in, it was decided by the team that "Treat warnings as errors" would be turned on for each of the projets so that any future warnings would result in failed builds. This now helps us keep the build clean.
...
In some scenarios, on a 64-bith version of Windows, migration of Meterpreter would sometimes result in the session terminating unexpectedly. This rather intimidating bug was the first I attempted to fix when I joined the project, and I documented the analysis and resolution [in a previous blog post][pointer_truncation], so if you're interested in the detail please have a read of that article. If your'e too lazy, the TL;DR version is that 64-bit pointers were being truncated to 32-bit as a result of a bad function pointer type declaration. This wasn't an issue until recently because it appears that new versions of [ASLR][] have just started allocating above the 4GB boundary.
...
``` C Definition of DISPATCH_ROUTINE https://github.com/rapid7/meterpreter/blob/master/source/common/base.h#L12
...
``` C Definition of INLINE_DISPATCH_ROUTINE https://github.com/rapid7/meterpreter/blob/master/source/common/base.h#L13
...
```C Inline Request Declaration https://github.com/rapid7/meterpreter/blob/master/source/common/base.c#L75
...
For this discussion, we'll stick to Windows and to the interactive channels which are responsible for dealing with shells. When an interactve channel is created that is tied to a background shell process a bunch of things happen:
...
1. The schedular is handed the detail of the channel so that the lifetime of the channel can be managed.
...
* The scheduler thread loop waited on the suspention object as well as the original two.
...
The guts of the work can be found in [this pull request][channel_refactor_pr]. There's actually not that much to it, but the nature of the changes were scary given that they messed with a pretty important part of the applciation.
...
This exploit is a gem from the might Tavis Ormandy, and was published in various places including the [Full Disclosure][kitrap0d_full_disclosure] mailing list a few years back.
...
![Yeah!](http://i.imgur.com/eyvNG.gif)
...
![Owned Windows](http://i.imgur.com/ifB9OsM.png)
