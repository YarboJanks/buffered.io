...
1. The two parameters, `off_804a050` and `off_804a05f` passed in are addresses to areas of memory, which implies that the call to the function is going to fill this memory with values.
...
The application is now waiting for input on `stdin`. If we type a couple of characters from the keyboard we'll see the application terminate. However, we know that it's running and we're ready to begin debuggin.
